...
use app\modules\admin\models\DownloadsSearch;
...
class DownloadsController extends Controller
{
    ...
    public function actionIndex()
    {
        if (Yii::$app->request->isAjax) {
            ...
            if (count($keys)) {
                foreach ($keys as $k => $v) {
                    if (($model = Downloads::findOne($v)) !== null) {
                        $model->delete();
                    }
                }
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            if (Yii::$app->request->post('Downloads')['source']) {
            $describe = $this->describeFile(Yii::$app->request->post('Downloads')['source']);
            $model->size = $describe['size'];
            $model->expansion = strtoupper($describe['expansion']);
            }else{
                $model->size = "No file";
                $model->expansion = "No file";
            }
            if ($model->save()) {
    ...
    private function describeFile($source)
    {
        $describe = [];
        $fullPath = Yii::getAlias('@webroot') . $source;
        $size = filesize($fullPath);
        if ($size < 1024) {
            $describe['size'] = intval($size) . ' B';
        } elseif ($size > 1024 && $size < 1048576) {
            $describe['size'] = intval($size / 1024) . ' KB';
        } elseif ($size > 1048576) {
            $describe['size'] = intval($size / 1048576) . ' MB';
        }
        $describe['expansion'] = end(explode(".", $source));
