...
class Generator extends \app\modules\gii\Generator
{
    ...
    public function rules()
    {
        return array_merge(parent::rules(), [
            [['db', 'ns', 'multiLang', 'tableName', 'modelClass', 'baseClass', 'queryNs', 'queryClass', 'queryBaseClass'], 'filter', 'filter' => 'trim'],
     ...
     * @param $mlFields
     */
    public function generateMultilangTableField($tableName,$mlFields)
    {
        $db = $this->getDbConnection();
        if ($db !== null) {
            $fildsLang=[];
            $fields = explode(' ',$mlFields);
            foreach($fields as $field){
                if(strlen($field)&&in_array(trim($field),$db->getTableSchema($tableName)->columnNames)){
                    $fildsLang[]=$field;
                    echo '\''.$field.'\',';
                }
            }
           $this->fildsLang = $fildsLang;
        }
    }
    /**
     * @inheritdoc
    ...
    public function generate()
    {
        ...
        foreach ($this->getTableNames() as $tableName) {
            ...
            if($this->multiLang&&$this->fildsLang){
                $mg_params = [];
                $tableSchema = $tableColumns = $db->getTableSchema($tableName);
                $tableColumns = $db->getTableSchema($tableName)->columns;
                foreach($tableColumns as $key=>$field){
                    if(!in_array($field->name,$this->fildsLang)&&!$field->isPrimaryKey){
                        unset($tableSchema->columns[$key]);
                    }
                }

                $db->createCommand('DROP TABLE IF EXISTS '.$tableName.'_lang')->execute();
                $mg = new Migration;
                foreach ($tableSchema->columns as $item) {
                    if($item->name=='id'){
                        $mg_params[$item->name]=$mg->primaryKey();
                    }else{
                        $type = $item->type;
                        $mg_params[$item->name]=$mg->$type($item->size)->null();
                    }
                }
                $mg_params['language']=$mg->string(255);
                $mg_params['original_id']=$mg->integer();

                $mg->createTable($tableName.'_lang',$mg_params);

                /*$sqlCreate='DROP TABLE IF EXISTS '.$tableName.'_lang;
                    CREATE TABLE '.$tableName.'_lang
(
`id` int(11) NOT NULL AUTO_INCREMENT,
`original_id` int(11),
`language` varchar(12) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL';
                foreach ($tableSchema->columns as $item) {
                    if(!$item->isPrimaryKey)
                    $sqlCreate.=', `'.$item->name.'` '.$item->dbType;
                }
...
ROW_FORMAT = Compact;';
                ...
                $params = [
                    'tableName' => $tableName.'_lang',
                    'className' => $modelClassName.'Lang',
                    'queryClassName' => $queryClassName,
                    'tableSchema' => $tableSchema,
                    'labels' => $this->generateLabels($tableSchema),
                    'rules' => $this->generateRules($tableSchema),
                ...
                $files[] = new CodeFile(
                    Yii::getAlias('@' . str_replace('\\', '/', $this->ns)) . '/' . $modelClassName . 'Lang.php',
                    $this->render('model_lang.php', $params)
                );
            }
            // query :
            if ($queryClassName) {
