...
class User extends ActiveRecord implements IdentityInterface
{
    ...
    {
        if ($this->last_activity == null || $this->last_activity >= time() - 60) return true;
    ...
    public function getRating($marks = false)
    {
        ...
        switch ($this->type) {
            case self::TYPE_DRIVER:
                $rating = (float)$this->driverRating($marks);
                break;

            case self::TYPE_PASSENGER:
                $rating = (float)$this->passengerRating($marks);
                break;
        }
    ...
    public function beforeSave($insert)
    {
        ...
        if ((float)$this->rating < 2.5 && $this->getRating(true) > 10 && intval($this->blocked_at) != 0) {
            ...
            Notifications::create(Notifications::NT_BLACKLIST, [$this->id], Yii::$app->params['blacklist']['rating']['notification']);
