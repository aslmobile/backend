...
$(function() {
    ...
    PcAlert = function(c, a, b) {
        ...
        $('#galleryerrors').html('<div class="alert alert-warning alert-dismissible"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button><h4><i class="icon fa fa-warning"></i> Alert!</h4>'+ c +'</div>');
    ...
    PcLoading = {
        ...
        show: function(c, a){
            $('#galleryLoading').html("<img src='" + __IMAGE_LOADING + "' >");
        },
        hide: function() {
            $('#galleryLoading').html("");
        }
    ...
    var methods = {
        ...
        init: function(Options) {
            ...
            var defaults = {
                ...
                EnableCrop: false,
                EnableResize: true,
                MinimumWidthToResize: 2600,
                MinimumHeightToResize: 1200,
                MaximumSize: 4096,
                EnableMaximumSize: true,
                PluginFolderOnServer: "/web/plugins/picturecut/",
                CropWindowStyle: "popstyle",
            ...
            return this.each(function() {
                ...
                var TuyoshiCrop = function(element, response) {
                    ...
                    /*$('#crpp').css({'transform':'scale(1)'});
                    if($(Options.Elem).width() < response.currentWidth){
                       var scale =  $(Options.Elem).width() / response.currentWidth;
                        $('#crpp').css({'transform':'scale('+scale+')'});
                    }*/


                    //$(Options.Elem).hide();

                    //console.log($(Options.Elem).width() +"===" +response.currentWidth+"====111");
                     /*
                     var rnw = response.currentWidth;
                     if($(Options.Elem).width() < response.currentWidth){
                     response.currentWidth = $(Options.Elem).width();
                     response.currentHeight = response.currentHeight*$(Options.Elem).width()/rnw;
                     }*/

                     /*
                    var rnw = response.currentWidth;
                    if($('#container_photo').width() < response.currentWidth){
                        response.currentWidth = response.currentWidth / 2;
                        response.currentHeight = response.currentHeight*response.currentWidth/rnw;
                    }
                    */
                    ...
                    var Carregar_Imagem = function(Principal, Imagem) {
                        ...
                        //response.currentWidth = response.currentWidth * scale;
                        //response.currentHeight = response.currentHeight * scale;
                        //console.log($(Options.Elem).width() +"===" +response.currentWidth+"===" +response.currentHeight+"====333");
                        */
                ...
                var Construir_Widget = function(element) {
                    ...
                    if($image.parent().attr('pic')!=''){ $image.attr('src',$image.parent().attr('pic'));}
                    if(Options.Editable){
                        element.append($('<a class="editPen"><i class="glyphicon glyphicon-edit"></i></a>'));
                    }
                ...
                var Setando_AjaxUpload = function(element) {
                    ...
                    //post["minimumWidthToResize"] = $(Options.Elem).width();
                    //post["minimumWidthToResize"] = 870;
