...
class PictureCut{
	...
	private function populateFromArray($data){
		...
		$ar = explode("/",$this->folderOnServer); $ar = array_diff($ar, array(''));
		$this->folderOnServer = '/web/files/gallery/'. array_pop($ar) .'/';
	...
	private function populateFileFromStream($fileStream){
		...
		if($this->imageNameRandom == "true"){
			...
			$newName = md5(uniqid().time()).".".$this->fileType;
	...
	public function upload(){
		...
		try {								
				if (move_uploaded_file($this->fileStream['tmp_name'], $this->currentFile)){
					...
					copy($this->currentFile,$this->folderPath."original/".$this->currentFileName);
